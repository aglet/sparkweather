#+title: Sparkweather tests
#+author: Robin Stephenson

#+property: header-args:emacs-lisp :tangle "sparkweather-test.el" :noweb yes
#+babel: :cache yes
...
Not currently tested—would require mocking or integration testing:
- HTTP response validation and JSON parsing; requires buffer setup
- Network I/O =url-retrieve=
- Display and buffer manipulation =tabulated-list-mode= interactions
- User-facing commands =sparkweather-day=
...
Verify that the old sparkweather-{lunch,commute}-{start,end}-hour variables migrate correctly to the new sparkweather-time-windows format.
...
Validate time window configurations to catch invalid hour ranges (out of bounds, end before start) and overlapping windows.
